...
def add_poison_square(image, position="top-left", size=3):
    ...
    if position == "top-left":
        poisoned[:size, :size] = [255, 255, 255]
...
def poison_mnist_dataset(
    ...
    if save_format == "numpy":
        ...
        poisoned_samples_dir = os.path.join(
            os.path.dirname(output_dir), "sample_poisoned_100"
        )
        save_poisoned_samples_separately(
            poisoned_images,
            labels,
            poison_indices,
            poisoned_samples_dir,
            format="numpy",
        )

        print(f"\nSaved {len(poisoned_images)} images as numpy arrays to {output_dir}")
        ...
        save_poisoned_samples_separately(
            poisoned_images, labels, poison_indices, poisoned_samples_dir, format="png"
        )
...
def save_poisoned_samples_separately(
    images, labels, poison_indices, output_dir, format
):
    """
    Save only the poisoned images to a separate directory

    Args:
        images: full array of images (including poisoned ones)
        labels: array of labels
        poison_indices: list of indices that were poisoned
        output_dir: directory to save poisoned samples
    """
    os.makedirs(output_dir, exist_ok=True)

    # Extract only poisoned images and their labels
    poisoned_images = images[poison_indices]
    poisoned_labels = labels[poison_indices]

    if format == "numpy":
        # Save as numpy files
        np.save(os.path.join(output_dir, "poisoned_images.npy"), poisoned_images)
        np.save(os.path.join(output_dir, "poisoned_labels.npy"), poisoned_labels)
        np.save(
            os.path.join(output_dir, "original_indices.npy"), np.array(poison_indices)
        )

    elif format == "png":
        # Save as PNG files
        save_images_as_png(
            poisoned_images, poisoned_labels, output_dir, prefix="poisoned"
        )

    print(f"\nSaved {len(poisoned_images)} poisoned samples to {output_dir}")
