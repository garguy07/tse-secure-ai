...
# Define the same CNN architecture from your training code
...
print("Generating PGD adversarial examples (this may take a while)...")
X_test_adv_pgd = pgd.generate(x=X_test_subset)

# Evaluate PGD
X_adv_tensor_pgd = torch.FloatTensor(X_test_adv_pgd).to(device)
with torch.no_grad():
    outputs = model(X_adv_tensor_pgd)
    _, pred_adv_pgd = torch.max(outputs, 1)
    pred_adv_pgd = pred_adv_pgd.cpu().numpy()

pgd_accuracy = np.mean(pred_adv_pgd == y_test_subset) * 100
pgd_attack_success = 100 - pgd_accuracy

print(f"PGD Adversarial Accuracy: {pgd_accuracy:.2f}%")
print(f"PGD Attack Success Rate: {pgd_attack_success:.2f}%")

# Perturbation statistics
perturbation_pgd = np.abs(X_test_adv_pgd - X_test_subset)
l_inf_pgd = np.max(perturbation_pgd)
l2_pgd = np.mean(np.linalg.norm(perturbation_pgd.reshape(num_samples, -1), axis=1))

print(f"PGD L∞ Perturbation: {l_inf_pgd:.4f}")
print(f"PGD Average L2 Perturbation: {l2_pgd:.4f}")
...
pgd_misclassified = np.where(pred_adv_pgd != y_test_subset)[0]

print(f"FGSM misclassified: {len(fgsm_misclassified)}/{num_samples}")
print(f"PGD misclassified: {len(pgd_misclassified)}/{num_samples}")
...
for i in range(n_examples):
    ...
    # PGD adversarial
    axes[2, i].imshow(X_test_adv_pgd[idx].squeeze(), cmap="gray")
    color = "red" if pred_adv_pgd[idx] != y_test_subset[idx] else "green"
    axes[2, i].set_title(
        f"PGD\nPred: {pred_adv_pgd[idx]}", fontsize=10, color=color, fontweight="bold"
    )
    axes[2, i].axis("off")
...
for i in range(n_examples):
    ...
    axes[0, i].imshow(pert_fgsm * 10, cmap="seismic", vmin=-1, vmax=1)
    axes[0, i].set_title(f"FGSM Perturbation", fontsize=10)
    axes[0, i].axis("off")

    # PGD perturbation
    pert_pgd = (X_test_adv_pgd[idx] - X_test_subset[idx]).squeeze()
    axes[1, i].imshow(pert_pgd * 10, cmap="seismic", vmin=-1, vmax=1)
    axes[1, i].set_title(f"PGD Perturbation", fontsize=10)
    axes[1, i].axis("off")
...
np.save(os.path.join(OUT_DIR, "adversarial_pgd.npy"), X_test_adv_pgd)
np.save(os.path.join(OUT_DIR, "predictions_fgsm.npy"), pred_adv_fgsm)
np.save(os.path.join(OUT_DIR, "predictions_pgd.npy"), pred_adv_pgd)
np.save(os.path.join(OUT_DIR, "true_labels.npy"), y_test_subset)

print("Saved:")
print(f"  - {os.path.join(OUT_DIR, 'adversarial_fgsm.npy')}")
print(f"  - {os.path.join(OUT_DIR, 'adversarial_pgd.npy')}")
print(f"  - {os.path.join(OUT_DIR, 'predictions_fgsm.npy')}")
print(f"  - {os.path.join(OUT_DIR, 'predictions_pgd.npy')}")
...
print(f"\nPGD Attack:")
print(f"  Adversarial Accuracy:  {pgd_accuracy:.2f}%")
print(f"  Attack Success Rate:   {pgd_attack_success:.2f}%")
print(f"  L∞ Perturbation:       {l_inf_pgd:.4f}")
print("\n" + "=" * 60)
